...
| 2026-09-26 12:38Z | Steps 1 to 3 are done for 0.3.0. The version places carry the same string; the READMEs carry no `Version <x>.` status line since the compact rewrite `74167b5`, so the places are five strings in four files: `pyproject.toml`, `__init__.py`, `<version>` and `<image-tag>` in `appinfo/info.xml`, and the self entry of `uv.lock`, this time raised by a real `uv sync` run. The changelog block `## [0.3.0] - 2026-09-26` grew out of the waiting `[Unreleased]` block, link definition paired, no `Unreleased` string left. All six gates green: 4428 tests passed with 33 skipped and 168 deselected, no lint finding, 275 files already formatted, no type error, no dead code, and the tool surface measures 17233 bytes across 22 tools against a budget of 18000, `files_download` being the 22nd. The local archive structure check gives the single top level folder `mcp_connector` and its `CHANGELOG.md` holds the `[0.3.0]` block. One environment note: a running `nc-mcp.exe` from this venv kept `uv sync` from replacing the console script stubs; the lock file was written regardless, and the published image is built in CI, not from this venv | `grep` over the five places, `grep -c 'Unreleased' CHANGELOG.md` gives 0, `uv run --no-sync` in front of each of the six gate commands, every one exit 0, then `bash scripts/build_store_release.sh` and `tar -tzf`/`tar -xzOf` over `dist/mcp_connector-0.3.0.tar.gz` |
| 2026-09-26 12:40Z | Step 4 and 5 are done for 0.3.0. The release commit `9acbaea` was pushed to the public `main` and then tagged; before it, the 72 commits of milestone v1.6 and the archive bookkeeping were already public with a green CI run `36241790877`. The release workflow of the tag `v0.3.0` is green, run `36242658121`: the multi arch image was pushed to `ghcr.io/street1983nk/mcp_connector:0.3.0` and the store archive was attached to the GitHub release as `mcp_connector-0.3.0.tar.gz` with 45858 bytes. The tag came into being after the explicit owner release of 2026-09-26 ("release 0.3.0 freigegeben") | `git push origin main`, `git tag v0.3.0`, `git push origin v0.3.0`, then `gh run watch 36242658121 --exit-status` exit 0 and `gh release view v0.3.0 --json assets` naming the asset at 45858 bytes with `isDraft` false |
| 2026-09-26 12:42Z | Step 6 is done for 0.3.0: the download answers 200 with 45858 bytes after a 302, and the signature over exactly those bytes verifies against the merged certificate: `Verified OK`. The downloaded asset carries the payload: its `CHANGELOG.md` holds the `[0.3.0]` block and its `info.xml` names `0.3.0` | `curl -sSLO` on the release asset URL, `openssl dgst -sha512 -sign` with the local key, `openssl x509 -pubkey` from the certificate and `openssl dgst -sha512 -verify` over the same file, then `tar -xzOf` over the changelog and the manifest |
| 2026-09-26 12:43Z | Step 7 is done for 0.3.0: the store answered `201` with an empty body. The submission ran in the page context of the signed in store session, the token never left the browser | `POST /api/v1/apps/releases` from `https://apps.nextcloud.com/account/token`, with the download URL and the freshly computed signature as the payload and `nightly` false |
| 2026-09-26 12:47Z | Step 8 is done for 0.3.0, all five proofs. The catalogue lists `0.3.0` with the span `>=32.0.0 <36.0.0` and our GitHub asset as the download, after the documented cache lag of about four minutes; the asset answers 200 with the signed 45858 bytes; the image is a real multi arch index with `linux/amd64` and `linux/arm64` next to the attestation entries; the registry tag list carries `0.3.0` with none rewritten; and `authors[0].mail` in the catalogue is `admin@infranode.dev`, the same string as `<author mail>` in the manifest. The span `>=32.0.0 <36.0.0` is the one the manifest has declared since 0.1.14 raised max-version to 35, and this is the first release that ships the token exchange path, off in the factory state | `curl -sS https://apps.nextcloud.com/api/v1/appapi_apps.json` twice, four minutes apart; `curl -sSIL` on the asset; `docker manifest inspect ghcr.io/street1983nk/mcp_connector:0.3.0`; the ghcr tags endpoint with an anonymous pull token; `grep` over `appinfo/info.xml` |
